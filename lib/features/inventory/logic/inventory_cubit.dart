...
class InventoryCubit extends Cubit<InventoryState> {
  InventoryCubit(this._inventorySummaryUseCase,this._InventoryProductUseCase) : super(InventoryState.initial());
  final InventorySummaryUseCase _inventorySummaryUseCase;
    final InventoryProudctUseCase _InventoryProductUseCase ;
  Future<void> getInventorySummary ( ) async {
    emit(InventoryState.loading());
  // i will handle this 
    final storeId = await SharedPrefHelper.getInt( SharedPrefHelper.storeIdKey, );
    final result = await _inventorySummaryUseCase.invoke(storeId);
    result.when(
      success: (data) => emit(InventoryState.success(data)),
      failure: (error) => emit(InventoryState.error(error)),
    );
  }
  Future<void> getInventoryProducts(String? itemType, int pageNumber, int pageSize) async {
    emit(InventoryState.loading());
    final storeId = await SharedPrefHelper.getInt(SharedPrefHelper.storeIdKey);
    final result = await _InventoryProductUseCase.invoke(storeId, itemType, pageNumber, pageSize);
    result.when(
      success: (data) => emit(InventoryState.success(data)),
      failure: (error) => emit(InventoryState.error(error)),
    );
